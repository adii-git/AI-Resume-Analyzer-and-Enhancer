...
Uses pdfplumber (primary) or PyMuPDF (fallback) for PDF.
Uses spaCy for name extraction when available, regex otherwise.
"""
import re, os
from typing import Dict, Any, List, Optional

# PDF — pdfplumber only (no Visual Studio required on Windows)
try:
    import pdfplumber
    _PDF = "pdfplumber"
except ImportError:
...
fitz = None  # PyMuPDF not used (requires C++ build tools on Windows)
...
TECH_SKILLS = {
    ...
    "data structures","algorithms","oop","unit testing","figma","jira","kafka","rabbitmq",
}
SOFT_SKILLS = {
...
_TITLE    = re.compile(r"(engineer|developer|analyst|manager|intern|scientist|architect|designer|consultant|lead|director|associate|specialist)", re.I)
_GPA      = re.compile(r"(gpa|cgpa)[:\s]*([\d.]+)", re.I)
_YEAR     = re.compile(r"\b(19|20)\d{2}\b")
_DATE     = re.compile(r"(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*[\s,]*\d{4}(\s*[-–]\s*(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*[\s,]*(\d{4}|present|current))?", re.I)
...
_SEC = {
    ...
    "experience"    : r"(experience|employment|work history|professional|career)",
...
class ResumeParser:
    ...
    def _pdf(self, path: str) -> str:
        ...
        if not text.strip():
            raise ValueError("No text extracted. PDF may be image-based or scanned.")
        return text
        ...
        for t in doc.tables:
            for row in t.rows:
                for cell in row.cells:
                    if cell.text.strip(): lines.append(cell.text)
        return "\n".join(lines)
    ...
    def _build(self, raw: str) -> Dict[str, Any]:
        ...
        skills = list(set(self._skills_in(secs.get("skills",[]) + secs.get("summary",[])) + self._skills_in_text(raw)))
        return {
            "raw_text"         : raw,
    ...
    def _name(self, lines: List[str], head: str) -> str:
        ...
        for l in lines[:5]:
            if pat.match(l) and not _EMAIL.search(l): return l
    ...
    def _hdr(self, line: str) -> Optional[str]:
        cl = line.strip().lower()
        if not (3 <= len(cl) <= 50) or cl.startswith(_BULLETS): return None
        for key, pat in _SEC.items():
            if re.search(pat, cl, re.I): return key
        return None

    def _skills_in(self, lines: List[str]) -> List[str]:
        text = " ".join(lines).lower()
        return [s.title() for s in (TECH_SKILLS | SOFT_SKILLS) if re.search(r"\b"+re.escape(s)+r"\b", text)]

    def _skills_in_text(self, text: str) -> List[str]:
        t = text.lower()
        return [s.title() for s in TECH_SKILLS if re.search(r"\b"+re.escape(s)+r"\b", t)]
    ...
    def _exp(self, lines: List[str]) -> List[Dict]:
        ...
        for line in lines:
            ...
            if _TITLE.search(line) and not is_b and len(line) < 100:
                if cur: entries.append(cur)
                cur = {"title": line.strip(), "company": "", "duration": "", "bullets": [], "quantified_impact": []}
            elif cur:
    ...
    def _proj(self, lines: List[str]) -> List[Dict]:
        ...
        for line in lines:
            is_b = line.startswith(_BULLETS)
            if not is_b and 2 < len(line.split()) <= 10:
                if cur: entries.append(cur)
                cur = {"title": line.strip(), "description": [], "tech_stack": [], "links": []}
            elif cur:
                if is_b: cur["description"].append(re.sub(r"^[•\-●▸∙*–→◦▪]\s*", "", line).strip())
                cur["links"].extend(_URL.findall(line))
                cur["tech_stack"].extend(self._skills_in([line]))
        if cur: entries.append(cur)
        if not entries and lines:
